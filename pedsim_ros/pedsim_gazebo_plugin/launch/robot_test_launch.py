...
def generate_launch_description():
    ...
    robot_gazebo_dir = get_package_share_directory('rur_detailed_description')
    ...
    worlds = 'cafe'
    # scene = 'office_small'
    # scene = 'office-cubicles'
    # scene = 'office_env_large'
    # scene = 'world_without_people'
    # scene = 'cafe1'
    scene = 'cafe2'

    world_file = os.path.join(pedsim_gazebo_dir, 'worlds', f'{worlds}.world')
    ...
    robot_state_publisher_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(robot_gazebo_dir, 'launch', 'robot_state_publisher.launch.py')
        ),
    ...
    spawn_robot_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(robot_gazebo_dir, 'launch', 'spawn_robot.launch.py')
        ),
        launch_arguments={
            'x_pose': x_pose,
            'y_pose': y_pose
    ...
    delayed_robot_and_pedsim = TimerAction(
    period=7.0,
    actions=[
        spawn_robot_cmd,
    ...
    ld.add_action(delayed_robot_and_pedsim)
