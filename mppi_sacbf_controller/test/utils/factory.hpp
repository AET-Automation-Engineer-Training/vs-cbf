...
std::shared_ptr<mppi_sacbf::Optimizer> getDummyOptimizer(
  TNode node, TCostMap costmap_ros,
  TParamHandler * params_handler)
{
  std::shared_ptr<mppi_sacbf::Optimizer> optimizer = std::make_shared<mppi_sacbf::Optimizer>();
...
mppi_sacbf::PathHandler getDummyPathHandler(
  TNode node, TCostMap costmap_ros, TFBuffer tf_buffer,
  TParamHandler * params_handler)
{
  mppi_sacbf::PathHandler path_handler;
  std::weak_ptr<rclcpp_lifecycle::LifecycleNode> weak_ptr_node{node};
...
std::shared_ptr<nav2_mppi_sacbf_controller::MPPISACBFController> getDummyController(
  TNode node, TFBuffer tf_buffer,
  TCostMap costmap_ros)
{
  auto controller = std::make_shared<nav2_mppi_sacbf_controller::MPPISACBFController>();
  std::weak_ptr<rclcpp_lifecycle::LifecycleNode> weak_ptr_node{node};
